import { NormalizedTransaction } from '../../../shared/types';
import { MAX_INITIAL_LOOKBACK_DAYS, MAX_PAGES_PER_SCAN } from '../../../shared/constants';
...
export class AlchemyProvider {
  ...
  private getEstimatedBlocksPerDay(baseUrl: string): number {
    ...
    if (baseUrl.includes('eth-mainnet')) return 7200; // ~12s
    if (baseUrl.includes('base-mainnet')) return 43200; // ~2s
    if (baseUrl.includes('polygon-mainnet')) return 43200; // ~2s
    if (baseUrl.includes('arb-mainnet')) return 345600; // ~0.25s
    return 7200;
  }

  async getTransfers(tokenAddress: string, lastScannedBlock: number): Promise<{ transfers: NormalizedTransaction[], nextBlockToScan: number }> {
    ...
    if (!fromBlock || fromBlock === 0) {
      ...
      fromBlock = Math.max(0, currentBlock - (MAX_INITIAL_LOOKBACK_DAYS * blocksPerDay));
    ...
    let transfers: NormalizedTransaction[] = [];
    let pageKey: string | undefined = undefined;
    let pagesFetched = 0;
    
    while (pagesFetched < MAX_PAGES_PER_SCAN) {
      const res = await this.fetchWithRetry(this.baseUrl, {
        id: 1,
        jsonrpc: "2.0",
