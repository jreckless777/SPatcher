...
export class PipelineService {
  ...
  private lockRepo = new LockRepository();
  private tokenRepo = new TokenRepository();
  private classificationService = new ClassificationService();

  async processScan(req: ScanTokenRequest): Promise<ScanTokenResponse> {
    ...
    try {
      ...
      // 4. Provider Fetch
      const provider = new AlchemyProvider(chainId);
      const { transfers, nextBlockToScan } = await provider.getTransfers(tokenId, token.lastScannedBlock);
      
      // 5. Load dedup state
      const processedTxSet = new Set<string>(token.lastScannedBlockTxIds || []);
      
      // 6. Load existing registry
      const existingRegistry = await this.tokenRepo.getRegistryMap(tokenId);

      // 7. Aggregate wallets & deduplicate
      const updatedRegistryMap = this.classificationService.aggregateWallets(
        transfers,
        poolAddress,
        existingRegistry,
        processedTxSet
