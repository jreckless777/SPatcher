...
const lockRepository_1 = require("./lockRepository");
const firebase_1 = require("../config/firebase");
async function cleanup(tokenId) {
    try {
        await firebase_1.db.collection('tokens').doc(tokenId).delete();
    }
    catch (error) {
        // Ignore error
    }
...
async function runTests() {
    ...
    const repo = new lockRepository_1.LockRepository();
    ...
    await firebase_1.db.collection('tokens').doc(tokenId).set({
        tokenId,
