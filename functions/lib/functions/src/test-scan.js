...
async function main() {
    const service = new pipelineService_1.PipelineService();
    const req = {
    ...
    const provider = new AlchemyProvider_1.AlchemyProvider(req.chainId);
    const currentBlock = await provider.getCurrentBlock();
    // Calculate block lookback similar to the provider
    const blocksPerDay = provider.getEstimatedBlocksPerDay(provider.baseUrl);
    ...
    console.log("lastScannedBlockTxIds:", res1.payload.token.lastScannedBlockTxIds);
    ...
    console.log("lastScannedBlockTxIds:", res2.payload.token.lastScannedBlockTxIds);
}
